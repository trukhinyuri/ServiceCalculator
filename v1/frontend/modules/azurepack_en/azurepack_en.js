...
(function() {
    ...
    function fillInputForm(i) {
        ...
        ram.value = parseFloat(ramValueString.replace(" GB.", ""));

        disk.focus();
        var diskValueString = azurepackServersTable.rows[i].cells[4].innerHTML;
        var diskValue = parseFloat(diskValueString.replace(" GB.", ""))
        disk.value = diskValue;

        snapshots.focus();
        var snapshotsValueString = azurepackServersTable.rows[i].cells[5].innerHTML;
        var snapshotsGbValue = parseFloat(snapshotsValueString.replace(" GB.", ""));
    ...
    function deleteTableItem(i) {
        ...
        var sum = parseFloat(sumString.replace("€", ""));
        ...
        var oldCostOfNonOptimalUtilizationRam = parseFloat(oldCostOfNonOptimalUtilizationRamString.replace("€", ""));

        var diffCostOfNonOptimalUtilizationRam = costOfNonOptimalUtilizationRam - oldCostOfNonOptimalUtilizationRam;

        azurepackInfrastructureTable.rows[2].cells[1].innerHTML = nonOptimalUtilizationRam + " GB.";
        azurepackInfrastructureTable.rows[2].cells[2].innerHTML = costOfNonOptimalUtilizationRam + " GB.";
    ...
    function addToList() {
        ...
        var costOfServer = (azurepackCalculator.getCostOfServer(azurepackServer) / 63).toFixed(2);
        ...
        cellServerName.appendChild(document.createTextNode(serverNameValue));
         ...
         if (regionValue == 0) {
             cellRegion.appendChild(document.createTextNode("Russia, Saint-Petersburg (SSD)"));
         } else if (regionValue == 1) {
             cellRegion.appendChild(document.createTextNode("Russia, Moscow (Tier III Gold, SSD + NLSAS СХД)"));
         } else if (regionValue == 2) {
             cellRegion.appendChild(document.createTextNode("Europe, Amsterdam (SSD)"));
         }
         ...
         cellRam.appendChild(document.createTextNode(ramValue + " GB."));

         var cellDisk = row.insertCell(4);
         cellDisk.appendChild(document.createTextNode(diskValue + " GB."));

         var cellSnapshots = row.insertCell(5);
        cellSnapshots.appendChild(document.createTextNode(snapshotsValue * diskValue + " GB."));

         var cellIPv4 = row.insertCell(6);
         cellIPv4.appendChild(document.createTextNode(ipv4Value));

         var cellServerCost = row.insertCell(7);
         cellServerCost.appendChild(document.createTextNode("€" + costOfServer));
    ...
    function updateAzurePackInfrastructureTable() {
        ...
        var costOfvLANs = (azurepackCalculator.getCostOfvLANs(azurepackAdditionalResources) / 63).toFixed(2);
        var costOfVPNs = (azurepackCalculator.getCostOfVPNs(azurepackAdditionalResources) / 63).toFixed(2);

        var nonOptimalUtilizationRam = checkNonOptimalUtilizationRam();
        var costOfNonOptimalUtilizationRam = (azurepackCalculator.getCostOfRam(1, nonOptimalUtilizationRam) / 63).toFixed(2);
        ...
        if ((azurepackInfrastructureTable.rows.length == 0) && (azurepackServersTable.rows.length != 0)) {
            ...
            resourceVLANName.appendChild(document.createTextNode("Virtual networks (vLAN)"));
            ...
            costVLAN.appendChild(document.createTextNode("€" + costOfvLANs));
            ...
            costVPN.appendChild(document.createTextNode("€" + costOfVPNs));
            ...
            resourceNonOptimalRamName.appendChild(document.createTextNode("Undistributed amount of RAM (vCPU cores should not be more than the amount of GB. RAM in the subscription)"));

            var countNonOptimalRam = nonOptimalMemoryRow.insertCell(1);
            countNonOptimalRam.appendChild(document.createTextNode(nonOptimalUtilizationRam + " GB."))

            var costNonOptimalRam = nonOptimalMemoryRow.insertCell(2);
            costNonOptimalRam.appendChild(document.createTextNode("€" + costOfNonOptimalUtilizationRam));
            ...
            var oldCostOfvLANs = parseFloat(oldCostOfvLANsString.replace("€", ""));

            var oldCostOfVPNsString = azurepackInfrastructureTable.rows[1].cells[2].innerHTML;
            var oldCostOfVPNs = parseFloat(oldCostOfVPNsString.replace("€", ""));
            ...
            azurepackInfrastructureTable.rows[0].cells[2].innerHTML = "€" + costOfvLANs;
            azurepackInfrastructureTable.rows[1].cells[1].innerHTML = VPNsValue;
            azurepackInfrastructureTable.rows[1].cells[2].innerHTML = "€" + costOfVPNs;
            azurepackInfrastructureTable.rows[2].cells[1].innerHTML = nonOptimalUtilizationRam + " GB.";
            azurepackInfrastructureTable.rows[2].cells[2].innerHTML = "€" + costOfNonOptimalUtilizationRam;
    ...
    function checkNonOptimalUtilizationRam() {
        ...
        for (var i = 0; i < azurepackServersTable.rows.length; i++) {
            ...
            sumRamGb += parseFloat(sumRamGbString.replace(" GB.", ""));
        }
    ...
    function clearLast() {
        ...
        var sumLast = parseFloat(sumLastString.replace("€", ""));
